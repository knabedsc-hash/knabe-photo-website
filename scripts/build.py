...
<body>
  ...
  <section class="contact-band" aria-label="研究者情報"><div class="container contact-inner"><div><p class="eyebrow">CONNECT</p><p>研究者情報・外部プロフィール</p></div><div class="footer-links"><a href="https://orcid.org/0000-0003-0827-7381" target="_blank" rel="noopener noreferrer">ORCID ↗</a><a href="https://researchmap.jp/waken-photo?lang=ja" target="_blank" rel="noopener noreferrer">researchmap ↗</a><a href="https://scholar.google.co.jp/citations?hl=ja&amp;view_op=list_works&amp;gmla=AHoSzlWOcv6OxsI3GazzPc9sTU87xCGglGpPowY-TIQJhBkRsllOkELvwh2yrKblA5T1ToyBhI6BQ6srQDuyRiWYpx4tVWUJ6Z4Wwbrcz_eLQ1U&amp;user=OOtNyewAAAAJ" target="_blank" rel="noopener noreferrer">Google Scholar ↗</a><a href="https://www.linkedin.com/in/kenta-watanabe-5ab557304/?locale=ja-JP" target="_blank" rel="noopener noreferrer">LinkedIn ↗</a></div></div></section>
...
en_home = f'''<section class="hero container"><div class="hero-copy">
  ...
  <p class="hero-description">I study photoelectrochemical phenomena involving light, electrons, and ions within solid materials and at solid surfaces, solid–solid interfaces, and solid–liquid interfaces, and develop materials for solar-energy conversion and storage.</p>
...
def teaching_sort_key(course):
    terms = [int(value) for value in re.findall(r"\d+", course["term"])]
    return terms[0], max(terms[1:], default=0)


teaching_courses = sorted(data["teaching"], key=teaching_sort_key, reverse=True)
teaching_jp = "".join(f'''<div><dt>{esc(course["term"].replace(",", "年度", 1).replace(",", "・"))}</dt><dd><h3>{esc(course["name"])}（{esc(course["code"])}）</h3><p lang="en">{esc(course["english"])}</p><p class="muted">{esc(course["affiliation"])}</p></dd></div>''' for course in teaching_courses)
...
teaching_en = "".join(f'''<div><dt>{esc(teaching_term_en(course["term"]))}</dt><dd><h3>{esc(course["english"])} ({esc(course["code"])})</h3><p class="muted">{english_affiliation(course["affiliation"])}</p></dd></div>''' for course in teaching_courses)
