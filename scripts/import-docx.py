"""Import local achievements references. Source documents are never published."""
...
teaching = [
    dict(term=group[0], code=group[1], name=group[2], english=group[3], affiliation=group[4])
    for group in (teaching_lines[index:index + 5] for index in range(0, len(teaching_lines), 5))
    if len(group) == 5
]
...
data["international"] = [record for record in data["international"] if "(Featured Speaker)" not in record[0] and "(Keynote)" not in record[0]]
